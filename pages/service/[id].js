...
export default function ServicePage() {
  ...
  useEffect(() => {
    if (!mounted || !service?.lat) return
    import('leaflet').then(L => {
      import('leaflet/dist/leaflet.css').then(() => {
        setMapComponent(() => () => {
          useEffect(() => {
            const map = L.default.map('service-map').setView([service.lat, service.lng], 15)
            ...
            L.default.marker([service.lat, service.lng], { icon }).addTo(map)
            return () => map.remove()
          }, [])
          return <div id="service-map" style={{ height: 220, borderRadius: 12, overflow: 'hidden' }} />
        })
      })
    })
  }, [mounted, service])
  ...
  return (
    <div dir="rtl" style={{ fontFamily: 'Arial, sans-serif', minHeight: '100vh', background: '#FFF8F3' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, #1A3A5C, #2A5298)`, color: 'white', padding: '32px', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ padding: '32px' }}>
            ...
            {service.lat && (
              <div style={{ marginBottom: 24 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: '#1A3A5C', marginBottom: 10 }}>📍 מיקום על המפה</div>
                <ServiceMap service={service} color={color} />
              </div>
            )}
