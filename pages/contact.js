    ...
    <>
      ...
      <div dir="rtl" style={{ fontFamily: "'Nunito', sans-serif", minHeight: '100vh', background: '#F8F9FF' }}>
          ...
          <nav style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {NAV.map(([href, label]) => (
              <a key={href} href={href} style={{
                color: 'white',
                background: href === '/contact' ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.1)',
                borderRadius: '999px', padding: '6px 14px', fontWeight: 600, fontSize: 12,
                border: href === '/contact' ? '1.5px solid rgba(255,255,255,0.6)' : '1.5px solid rgba(255,255,255,0.2)',
                textDecoration: 'none',
              }}>{label}</a>
            ))}
          </nav>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1A3A5C, #2A5298)', color: 'white', padding: '48px 20px', textAlign: 'center' }}>
          <img src='/contact-icon.png' alt='צור קשר' style={{ width: 120, height: 120, objectFit: 'contain', marginBottom: 0, filter: 'invert(1) brightness(10)' }} />
        ...
        <main style={{ maxWidth: 620, margin: '0 auto', padding: '40px 16px 64px' }}>
          {sent ? (
            <div style={{ background: 'white', borderRadius: 20, padding: '52px 32px', textAlign: 'center', boxShadow: '0 4px 24px rgba(26,58,92,0.1)', border: '1.5px solid #C5D0F0' }}>
              ...
              <div style={{ fontSize: 15, color: '#666', marginBottom: 28, lineHeight: 1.6 }}>תודה על הפנייה. נעשה כמיטב יכולתנו להגיב בהקדם.</div>
              <a href="/" style={{ display: 'inline-block', background: '#1A3A5C', color: 'white', borderRadius: '999px', padding: '12px 28px', fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>חזרה לדף הבית</a>
            </div>
          ) : (
            <div style={{ background: 'white', borderRadius: 20, padding: '32px 28px', boxShadow: '0 4px 24px rgba(26,58,92,0.1)', border: '1.5px solid #C5D0F0' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {TYPES.map(t => (
                    <label key={t.value} style={{
                      ...
                      background: type === t.value ? '#EEF2FF' : '#F8F9FF', transition: 'all 0.15s',
                    }}>
                      <input type="radio" name="type" value={t.value} checked={type === t.value} onChange={() => setType(t.value)} style={{ accentColor: '#1A3A5C', width: 16, height: 16 }} />
                      <span style={{ fontSize: 14, fontWeight: type === t.value ? 700 : 500, color: type === t.value ? '#1A3A5C' : '#555' }}>{t.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              {error && (
                <div style={{ background: '#FFF0F0', border: '1px solid #FFCDD2', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#C62828', marginBottom: 16 }}>⚠️ {error}</div>
              )}

              <button onClick={handleSubmit} disabled={sending} style={{
                width: '100%', background: sending ? '#aaa' : '#1A3A5C', color: 'white', border: 'none', borderRadius: 20,
        ...
        <footer style={{ background: '#1A3A5C', color: 'rgba(255,255,255,0.7)', textAlign: 'center', padding: '24px', fontSize: 13 }}>
          <div style={{ marginBottom: 8 }}>
            {[['/', 'ראשי'], ['/rehab', 'שיקום'], ['/treatment', 'טיפול'], ['/map', 'מפה'], ['/guide', 'מדריך'], ['/contact', 'צור קשר'], ['/about', 'אודות']].map(([href, label], i, arr) => (
              <span key={href}>
