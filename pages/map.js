...
export default function MapPage() {
  ...
  useEffect(() => {
      ...
      practitioners
        ...
        .forEach(p => {
          ...
          const icon = L.divIcon({
            html: `<div style="position:relative;width:16px;height:22px"><div style="width:16px;height:16px;border-radius:50% 50% 50% 0;background:${PRACT_COLOR};transform:rotate(-45deg);border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.35)"></div></div>`,
            className: '', iconSize: [16, 22], iconAnchor: [8, 22],
          })
    ...
    const updateViewport = () => {
      ...
      const inBounds = (lat, lng) => lat && lng && b.contains([lat, lng])
      const vItems = []
      if (showRehab) {
        rehabServices.forEach(s => { if (inBounds(s.lat, s.lng)) vItems.push({ ...s, _vtype: 'rehab' }) })
      }
      if (showTreatment) {
        treatmentServices.forEach(s => { if (inBounds(s.lat, s.lng)) vItems.push({ ...s, _vtype: 'treatment' }) })
      }
      if (showPractitioners) {
        practitioners.filter(p => p.city && CITY_COORDS[p.city]).forEach(p => {
          const [lat, lng] = CITY_COORDS[p.city]
          if (inBounds(lat, lng)) vItems.push({ ...p, _vtype: 'practitioner' })
        })
      }
      setViewportItems(vItems)
    }

    mapRef.current.off('moveend').on('moveend', updateViewport)
    ...
    <>
      ...
      <div dir="rtl" style={{ fontFamily: "'Nunito', sans-serif", minHeight: '100vh', background: '#f5f5f5', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          ...
          <div style={{ background: 'white', borderBottom: '1px solid #e0e0e0', padding: '10px 16px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              <input type="text" placeholder="מעוף לעיר..." value={cityInput} onChange={e => setCityInput(e.target.value)}
                ...
                style={{ padding: '7px 32px 7px 12px', borderRadius: '999px', border: `1.5px solid ${cityInput ? PRACT_COLOR : '#ddd'}`, fontSize: 13, fontFamily: "'Nunito', sans-serif", outline: 'none', width: 160, direction: 'rtl' }} />
              {cityInput && Object.keys(CITY_COORDS).filter(c => c.includes(cityInput)).slice(0,5).length > 0 && (
                <div style={{ position: 'absolute', top: '100%', right: 0, background: 'white', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.12)', zIndex: 600, minWidth: 160, marginTop: 4 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <button aria-pressed={showPractitioners} onClick={() => setShowPractitioners(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: '999px', border: `2px solid ${showPractitioners ? PRACT_COLOR : '#ddd'}`, background: showPractitioners ? '#e0f0ff' : 'white', color: showPractitioners ? PRACT_COLOR : '#aaa', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: "'Nunito', sans-serif" }}>
                <div style={{ width: 10, height: 10, borderRadius: '50% 50% 50% 0', transform: 'rotate(-45deg)', background: showPractitioners ? PRACT_COLOR : '#ddd' }} />🧠 מטפלים פרטיים
              </button>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => setShowPractitioners(v => !v)} style={{ flex: 1, padding: '10px', borderRadius: 12, border: `2px solid ${showPractitioners ? PRACT_COLOR : '#ddd'}`, background: showPractitioners ? '#e0f0ff' : 'white', color: showPractitioners ? PRACT_COLOR : '#aaa', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: "'Nunito', sans-serif" }}>🧠 מטפלים</button>
                  </div>
                </div>
                {/* קטגוריית שיקום */}
                {showRehab && (
        ...
        <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* ── Sidebar viewport ── */}
          {sidebarOpen && (
            <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: isMobile ? '100%' : 300, background: 'rgba(255,255,255,0.97)', zIndex: 500, overflowY: 'auto', boxShadow: '-3px 0 16px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '10px 14px', borderBottom: '1px solid #eef2f7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', flexShrink: 0 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#1A3A5C' }}>
                  {viewportItems.length > 0 ? `${viewportItems.length} באזור הנוכחי` : 'גללו על המפה לתוצאות'}
                </span>
                <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', fontSize: 16, cursor: 'pointer', color: '#888', padding: 0 }}>✕</button>
              </div>
              {[
                { type: 'rehab', label: '♿ שיקום', color: '#8B00D4' },
                { type: 'treatment', label: '🏥 טיפול', color: '#0891B2' },
                { type: 'practitioner', label: '🧠 מטפלים פרטיים', color: PRACT_COLOR },
              ].map(group => {
                const items = viewportItems.filter(i => i._vtype === group.type)
                if (!items.length) return null
                return (
                  <div key={group.type}>
                    <div style={{ padding: '8px 14px 4px', fontSize: 11, fontWeight: 800, color: group.color, background: '#f8fafc', borderBottom: '1px solid #eef2f7', letterSpacing: 0.3 }}>
                      {group.label} ({items.length})
                    </div>
                    {items.slice(0, 20).map(s => {
                      const isSelected = selected?.id === s.id && selected?.type === group.type
                      const color = group.type === 'rehab' ? (REHAB_COLORS[s.category] || '#8B00D4') : group.type === 'treatment' ? (TREATMENT_COLORS[s.category] || '#0891B2') : PRACT_COLOR
                      const coords = group.type === 'practitioner' && s.city ? CITY_COORDS[s.city] : (s.lat ? [s.lat, s.lng] : null)
                      return (
                        <div key={s.id + group.type}
                          onClick={() => {
                            setSelected({ ...s, type: group.type })
                            if (coords && mapRef.current) mapRef.current.flyTo(coords, 14, { duration: 0.6 })
                          }}
                          style={{ padding: '9px 14px', cursor: 'pointer', borderBottom: '1px solid #f0f4f8', background: isSelected ? '#f0f7ff' : 'white', borderRight: `3px solid ${isSelected ? color : 'transparent'}`, transition: 'background .1s' }}
                          onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = '#f8fafc' }}
                          onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = isSelected ? '#f0f7ff' : 'white' }}
                        >
                          <div style={{ fontWeight: 700, fontSize: 12, color: '#1A3A5C', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</div>
                          <div style={{ fontSize: 11, color: '#888', display: 'flex', gap: 5 }}>
                            {s.city && <span>📍 {s.city}</span>}
                            {(s.category || s.profession) && <span style={{ color, fontWeight: 600 }}>{s.category || s.profession}</span>}
                          </div>
                        </div>
                      )
                    })}
          ...
          {!sidebarOpen && (
            <button onClick={() => setSidebarOpen(true)} style={{ position: 'absolute', top: 12, right: 12, zIndex: 400, background: 'white', border: 'none', borderRadius: 999, padding: '7px 14px', fontWeight: 700, fontSize: 12, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', fontFamily: "'Nunito',sans-serif", color: '#1A3A5C' }}>
              📋 {viewportItems.length} באזור
            </button>
          )}

          {/* ── Legend ── */}
          <div style={{ position: 'absolute', bottom: 40, left: 16, background: 'rgba(255,255,255,0.95)', borderRadius: 10, padding: '7px 12px', boxShadow: '0 2px 10px rgba(0,0,0,0.12)', zIndex: 400, fontSize: 11, fontFamily: "'Nunito',sans-serif", lineHeight: 1.9, direction: 'rtl' }}>
          ...
          {selected && (
            <div style={{
              position: 'absolute',
              bottom: 0,
              right: sidebarOpen && !isMobile ? 300 : 0,
              left: 0,
              ...
              {selected.type === 'practitioner' ? (
                <div style={{ display: 'flex', gap: 8 }}>
                  <a href={`/practitioner/${selected.id}`} style={{ flex: 1, display: 'block', textAlign: 'center', background: PRACT_COLOR, color: 'white', borderRadius: '999px', padding: '8px 0', fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>לפרטים המלאים ←</a>
